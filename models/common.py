...
def apply_mix(hidden_states, mixstyle_layer):
    # Extract visual features (excluding the class token)
    x_visual = hidden_states[:, 1:197, :]
    # Reshape for MixStyle application
    x_visual = x_visual.permute(0, 2, 1)
    dim0, dim1, dim2 = x_visual.shape
    new_dim2 = int(math.sqrt(dim2))
    x_visual = x_visual.reshape(dim0, dim1, new_dim2, -1)  # [batch, 768, 14, 14]

    # Apply MixStyle
    x_visual = mixstyle_layer(x_visual)

    # Reshape back to original form
    x_visual = x_visual.reshape(x_visual.shape[0], x_visual.shape[1], -1)  # [batch, 768, 196]
    x_visual = x_visual.permute(0, 2, 1).to(hidden_states.dtype)  # [batch, 196, 768]

    # Concatenate with the class token
    processed_hidden_states = torch.cat([hidden_states[:, :1, :], x_visual], dim=1)
